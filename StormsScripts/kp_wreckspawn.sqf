...
//Added by storm; seed to use when placing wrecks. Change this is if you want the vehicles spawn pos to change.
_wreckSeed = 0;
...
while {(count _roadsArray) > 0} do {
	_road = _roadsArray select (count _roadsArray)-1;
	//Storm added a seed to make it spawn same wrecks each time
	_roadsArray = _roadsArray select {_x distance2D _road > (ceil (( _wreckSeed random (_maxDist - _minDist)) + _minDist))};
	_count = 0;
	while {(_count == 0) || ((_count < _multiMax) && ((_wreckSeed random 100) <= _multiChance))} do {
		_position = (getPos _road) findEmptyPosition [1, 15, "B_MRAP_01_F"];
		if !(_position isEqualTo []) then {
			_wreck = (_wrecksArray select (_wreckSeed random (count _wrecksArray))-1) createVehicle _position;
			_wreck setDir (round (_wreckSeed random 360));
